...
export default class Templates {
    ...
    async getTemplates() {
        ...
        this.__templates = response.data.templates;

        const html = response.data.templates.reduce((acc, value) => {
    ...
    async createTemplate(title, file) {
    ...
    __selectHandler() {
        ...
        const html = this.__templates.reduce((acc, value) => {
            acc += `<option data-id="${value.id}">${value.title}</option>`;
